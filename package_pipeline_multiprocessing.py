...
from pipeline_transform_spaGCN_embedding_to_image_backup import transform_embedding_to_image
...
def load_data1(h5_path, spatial_path, scale_factor_path):
    # Read in gene expression and spatial location
    adata = sc.read_10x_h5(h5_path)
...
def pseudo_images(h5_path, spatial_path, scale_factor_path, output_folder,method, panel_gene_path, pca_opt, transform_opt):
    ...
    if method == 'spaGCN':
        ...
        scgnnsp_PEalphaList = [ '1.0']#1.0, '0.2', '0.3', '0.5', '1.0', '1.2', '1.5', '2.0']
        scgnnsp_zdimList = ['3']#3, '10',  '16',  '32', '64', '128', '256']
        ...
        for scgnnsp_zdim in scgnnsp_zdimList:
            for scgnnsp_alpha in scgnnsp_PEalphaList:
                tmp=pool.apply_async(pseduo_images_scGNN, (h5_path, spatial_path, scale_factor_path, output_folder,
