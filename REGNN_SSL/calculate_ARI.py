...
#from REGNN_SSL.pipeline_transform_embedding_to_image import get_ground_truth, get_clusters
from REGNN_SSL.generate_embedding import generate_embedding_sc
import random, torch
import warnings
    ...
    for scgnnsp_dist in scgnnsp_knn_distanceList:
            for scgnnsp_k in scgnnsp_kList:
                    ...
                    column_name = 'PEalpha' + str(scgnnsp_alpha) + '_zdim' + str(
                        scgnnsp_zdim)
                    embedding = generate_embedding_sc(adata, sample=sample, scgnnsp_dist=scgnnsp_dist, scgnnsp_alpha=scgnnsp_alpha, scgnnsp_k=scgnnsp_k,scgnnsp_zdim=scgnnsp_zdim, scgnnsp_bypassAE=scgnnsp_bypassAE, geneSelectnum=gene_num)
                    # embedding = embedding.detach().numpy()
                    #embedding = pd.read_csv('result/Embeddings/'+image_name+'_embedding_SSLfinal.csv',names =['embedding0', 'embedding1', 'embedding2'], header=None)
                    ...
                    if (embedding != embedding).any() == True:
                        ...
                        if not os.path.exists('result/'):
                            os.makedirs('result/')
                        if not os.path.exists('result/all_results_SSL.csv'):
                            ARI_result.to_csv('result/all_results_SSL.csv', index=True, header=True,mode = 'a')
                        else: 
                            ARI_result.to_csv('result/all_results_SSL.csv', index=True, header=False,mode = 'a')
                        
                        table = pd.read_csv('result/ARI_table_SSL.csv', index_col=0)
                        table.at[sample,column_name] = ARI
                        table.to_csv('result/ARI_table_SSL.csv', index=True, header=True, mode = 'w')
...
def calc_ARI_SSL(args):
    ...
    for i in scgnnsp_zdimList:
        for j in scgnnsp_PEalphaList:
            ...
            df[column_name] = None
    df.to_csv('REGNN_SSL/result/ARI_table_SSL.csv', index=True, header=True, mode = 'w')
