...
import argparse
import scanpy as sc
from REGNN_SSL.src.graph_func import graph_construction
from REGNN_SSL.src.SEDR_train import SEDR_Train
import random, torch
import pandas as pd
import os
import shutil
...
def generate_embedding_sc(anndata, sample, scgnnsp_dist, scgnnsp_alpha, scgnnsp_k, scgnnsp_zdim, scgnnsp_bypassAE, geneSelectnum):
    scGNNsp_folder = "REGNN_SSL/scGNNsp_space/"
    ...
    scgnnsp_output_embedding_csv = datasetName + '_' + scgnnsp_k + '_' + scgnnsp_dist + '_NA_dummy_add_' + scgnnsp_alpha + '_intersect_160_GridEx19_embedding.csv'                                                                                                                                             #GaeHidden 3....                                             dummy
    command_scgnnsp = 'python -W ignore scGNNsp_alt.py --datasetName ' + datasetName + ' --datasetDir ./  --outputDir ' + scgnnsp_output_folder + ' --resolution 0.3 --nonsparseMode --EM-iteration 1 --useSpatial --model PAE --useGAEembedding --saveinternal --no-cuda --debugMode savePrune --saveinternal --GAEhidden2 16 --prunetype spatialGrid --PEtypeOp add --pe-type dummy'
