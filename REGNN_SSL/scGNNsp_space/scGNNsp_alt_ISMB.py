    ...
    for batch_idx, (data, dataindex) in enumerate(train_loader):
        ...
        # Classical usage with Feature autoEncoder
            ...
            # positional AE
                ...
                print("spatialMatrixBatch: ", spatialMatrixBatch.shape)
                #print("spatialMatrixBatch: ", spatialMatrixBatch)
                # decode
                recon_batch = model(spatialMatrixBatch, z).view(B, -1)
...
if __name__ == "__main__":
    ...
    if args.debugMode == 'loadPrune':
        ...
        # print('Mem consumption: '+str(mem))
    """
    adj_img = np.loadtxt('/N/slate/mraina/RESEPT_EGNN/scGNNsp_space/img_data/adj_057_weighted_rgb.csv', delimiter=',').reshape(407,1)
    #print("adj_img", adj_img[:5, :])
    adj_img = normalize(adj_img, axis=0)
    #print("adj_img",adj_img[:5, :])
    zOut = np.concatenate((zOut, adj_img), axis=1)
    print("zOut After Concat: ", zOut)
    print(zOut.shape)
    """
    print("zOut Before SSL: ", zOut)
    print(zOut.shape)

    if args.embedding=='GAE' or args.embedding=='Both':
        # Whether use GAE embedding
        if args.useGAEembedding or args.useBothembedding:
            print("zOut before GAE: ", zOut.shape)
            print(zOut[:5, :])
            zDiscret = zOut > np.mean(zOut, axis=0) # normalize(zOut, axis=0) 
            print("zOut to zDiscret: ", zDiscret.shape)
            print(zDiscret[:5, :])
            zDiscret = 1.0*zDiscret
            #zDiscret = np.concatenate((zDiscret, adj_img), axis=1)
            #print("zDiscret After Concat: ", zDiscret)
            #print(zDiscret.shape)
            print("New zDiscret", zDiscret[:5, :])
            #print("zDiscrete: ", zDiscret)
        ...
        # Adding this block to handle when embedding includes NaN to prevent code from breaking
