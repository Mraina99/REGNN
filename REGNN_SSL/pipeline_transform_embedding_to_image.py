...
def get_ground_truth(anndata,sample_name,img_folder, img_type, scale_factor_file= None):
    ...
    for x,y in zip(anndata.obs["array_row"],anndata.obs["array_col"]):
        c = color_get(str(label_index[count]))
        #print("c: ", c)
        plt.scatter(x/100,y/100,color=c,s=20)
...
def color_get(val):
    ...
    if val.isdigit() == True:
        ...
        if val == "epithelial cells":
            return color_list[0] # 0 - red
        elif val == "endothelial cells":
            return color_list[1] # 1 - green
        elif val == "stroma cells":
            return color_list[2] # 2 - yellow
        elif val == "immune cells":
            return color_list[3] # 3 - blue
        elif val == "neural cells":
            return color_list[4] # 4 - orange
