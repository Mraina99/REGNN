...
from model import AE, VAE, PVAE, PAE, GCN, SAGE, DGCNN, GIN
...
# The code has not cleaned yet
# Part of the codes from scGNN
# Part of the codes from GraphCL https://github.com/Shen-Lab/GraphCL
parser = argparse.ArgumentParser(description='Main entrance of self supervised learning scGNN for spatial transcriptomics')
parser.add_argument('--datasetName', type=str, default='151671',
                    help='For 10X: folder name of 10X dataset; For CSV: csv file name')
parser.add_argument('--datasetDir', type=str, default='/Users/wangjue/workspace/scGNNsp/',
                    help='Directory of dataset: default(/storage/htc/joshilab/wangjue/casestudy/)')

parser.add_argument('--batch-size', type=int, default=12800, metavar='N',
                    help='input batch size for training the graph (default: 1): Here temporarily load the whole graph in one batch. Original is 12800')
...
parser.add_argument('--prunetype', type=str, default='spatialGrid',
                    help='prune type, KNNgraphStats/KNNgraphML/KNNgraphStatsSingleThread/spatialGrid/Lattice (default: KNNgraphStatsSingleThread for scRNA-seq)')
...
parser.add_argument('--embedding', type=str, default='SSL',
                    help='Embedding Type: SSL/GAE/Both/None')
parser.add_argument('--n-clusters', default=7, type=int,
...
# spatial related
# False
parser.add_argument('--useSpatial', action='store_true', default=True,
...
parser.add_argument('--usePCA', action='store_true', default=False,
                    help='whether use PCA instead of Feature AE (default: False)')
...
# Spatail Exploration with various edges
parser.add_argument('--MVstrategy', choices=('SandC','C2S','NA','hybrid','spatial'), default='spatial', help='Type of MultiView integration (default: %(default)s)') #Original useMultiView
...
parser.add_argument('--SpruneTag', type=str, default='GridEx', help='Spatial Prunetype GridEx19 (default: %(default)s)')
...
parser.add_argument('--hidunits',         type=int,           default=16,                 help='num of hid units')
...
# Preprocessing
# if use PCA/raw expression as the first step, use args.usePCA or args.bypassAE will disable these autoencoders
if args.model == 'VAE':
    model = VAE(dim=scData.features.shape[1]).to(device)
elif args.model == 'AE':
    model = AE(dim=scData.features.shape[1]).to(device)
elif args.model == 'PVAE':
    # Results are very bad for varational Autoencoder
...
def train(epoch, train_loader=train_loader, EMFlag=False, taskType='celltype', sparseImputation='nonsparse'):
    ...
    '''
    if not args.bypassAE and not args.usePCA:
        model.train()
    train_loss = 0
    for batch_idx, (data, dataindex) in enumerate(train_loader):
        ...
        if args.bypassAE:
            recon_batch = data
            z = data
            print('Use original dataset in size: '+str(z.shape))

        # use PCA instead feature autoEncoder to accelerate
        elif args.usePCA:
            dataUse = data.detach().cpu().numpy()            
            pca = PCA(n_components=args.zdim)
            z = pca.fit_transform(dataUse)
            recon_batch = data
        ...
        # Classical usage with Feature autoEncoder
            ...
            # for spatial analysis, incorperating spatial information
            # TODO: integrate Hist image in spatial transcriptomics
            if args.useSpatial:
            ...
            # For taskType: celltype/imputation, Now imputation only works fro scRNA-seq
...
if __name__ == "__main__":
    ...
    if args.debugMode == 'savePrune' or args.debugMode == 'noDebug':
        ...
        # for bypass feature AE, or use PCA, only run onces
        if args.bypassAE or args.usePCA:
            epochs = 2
        ...
        # Step 1. Inferring celltype in scRNA-seq
        ...
        if args.adjtype == 'unweighted': 
            ...
            if args.MVstrategy == 'hybrid':
                ...
                adjBasicS, edgeListBasicS = generateAdj(zOut, graphType='spatialGrid', para='euclidean:8:Grid', adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
                print(len(edgeListBasicS))
                adjBasicSe, edgeListBasicSe = generateAdj(zOut, graphType='spatialGrid', para='euclidean'+':'+args.Sk+':'+args.SpruneTag, adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
                print(len(edgeListBasicSe))
                adjBasicC, edgeListBasicC = generateAdj(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k)+':'+args.pruneTag, adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix) 
            ...
            # Original of spatial Graph and correlation Graph,single modal                    
            elif args.MVstrategy == 'NA':
                print('Using original MVstrategy')
                adj, edgeList = generateAdj(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k)+':'+args.pruneTag, adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
            
            # exclusive Spatial Graph, part related with 'NA'
            elif args.MVstrategy == 'spatial':
                print('Using spatial Grid in MVstrategy')
                adj, edgeList = generateAdj(zOut, graphType='spatialGrid', para='euclidean:8:Grid', adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
            ...
            # MultiView
                ...
                if args.MVGrid == 'ori': # Prune: only use nearest neighbor as exact grid: 6 in cityblock, 8 in euclidean
                    adjTarget, edgeListT = generateAdj(zOut, graphType='spatialGrid', para='euclidean:8:Grid', adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
                    
                elif args.MVGrid == 'ex': # Prune: only use nearest/second nearest neighbor as exact grid: 12 in cityblock, 16 in eculidean
                    adjTarget, edgeListT = generateAdj(zOut, graphType='spatialGrid', para='euclidean:16:GridEx', adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
                adjSource, edgeListS = generateAdj(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k)+':'+args.pruneTag, adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)    
        ...
        # It looks like does not work in weighted
        elif args.adjtype == 'weighted':
            adj, edgeList = generateAdjWeighted(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k), adjTag=(args.embedding!='None'), spatialMatrix = spatialMatrix)
    ...
    # Whether use embedding
    if args.embedding=='GAE' or args.embedding=='Both':
        zDiscret = zOut > np.mean(zOut, axis=0)
        zDiscret = 1.0*zDiscret
        if args.embedding=='GAE':
            # mem = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            # print('Mem consumption: '+str(mem))
            if args.MVstrategy == 'C2S':
 ...
 ######################################   
    ...
    for bigepoch in range(0, args.EM_iteration):
        ...
        if args.clustering_method == 'Louvain':
            ...
            listResult = clustering.predict(zOut)
        #TODO
        elif args.clustering_method == 'deepcluster':            
            clustering = KMeans(n_clusters=args.n_clusters,
                                random_state=0).fit(zOut)
            listResult = clustering.predict(zOut)
        elif args.clustering_method == 'swav':            
            clustering = KMeans(n_clusters=args.n_clusters,
                                random_state=0).fit(zOut)
            listResult = clustering.predict(zOut)    
        else:
        ...
        if args.adjtype == 'unweighted':
            if args.MVstrategy == 'C2S':
                ...
                adjSource, edgeList = generateAdj(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k)+':'+args.pruneTag, adjTag=(args.embedding=='GAE' or args.embedding=='Both' or (bigepoch == int(args.EM_iteration)-1)), spatialMatrix = spatialMatrix)    
            if args.MVstrategy == 'SandC':
                # TODO not optimize for iteration now
                print("TODO")
            elif args.MVstrategy == 'NA':
                adj, edgeList = generateAdj(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k)+':'+args.pruneTag, adjTag=(args.embedding=='GAE' or args.embedding=='Both' or (bigepoch == int(args.EM_iteration)-1)), spatialMatrix =spatialMatrix)
        
        elif args.adjtype == 'weighted':
            adj, edgeList = generateAdjWeighted(zOut, graphType=args.prunetype, para=args.knn_distance+':'+str(args.k), adjTag=(args.embedding=='GAE' or args.embedding=='Both' or (bigepoch == int(args.EM_iteration)-1)), spatialMatrix =spatialMatrix)
    ...
    embedding_df.to_csv(save_name+'_embedding.csv')
    graph_df = pd.DataFrame(edgeList, columns=["NodeA", "NodeB", "Weights"])
    graph_df.to_csv(save_name+'_graph.csv', index=False)
    ...
    results_df.to_csv(save_name+'_results.txt')
