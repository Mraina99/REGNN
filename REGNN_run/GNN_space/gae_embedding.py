...
from gae.model import GCNModelVAE, GCNModelAE, GCNModelVAE_old
...
def GAEembedding(z, adj, spatialMatrix, edges, args):
    '''
    ...
    if args.GAEmodel == 'gcn_vae':
        model = GCNModelVAE(feat_dim, args.GAEhidden1, args.GAEhidden2, args.GAEdropout) #egnn in GCNModelVAE
    else:
    ...
    for epoch in tqdm(range(args.GAEepochs)):
        ...
        model.train()
        optimizer.zero_grad()
        z, mu, logvar = model(features, adj_norm, spatialMatrix,edges)
    ...
    if (hidden_emb != hidden_emb).any() == True:
        print("Embedding has NaN values.")
        print("Hidden EMbedding length: ", hidden_emb.size)
        print("Hidden EMbedding NaN: ", np.count_nonzero(np.isnan(hidden_emb)))
        return hidden_emb
    else:
